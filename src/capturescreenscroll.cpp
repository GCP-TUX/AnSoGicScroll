...
bool captureScreenScroll::imagesEqual(const QImage& a, const QImage& b)
{
    ...
    // Solo comparar la franja inferior donde entraría contenido nuevo
    int stripHeight = a.height() * 0.15;
    cv::Rect roi(0, a.height() - stripHeight, a.width(), stripHeight);

    cv::Mat diff;
    cv::absdiff(imgA(roi), imgB(roi), diff);
    cv::cvtColor(diff, diff, cv::COLOR_BGRA2GRAY);

    // Contar % de píxeles realmente distintos, no solo el promedio
    cv::Mat mask;
    cv::threshold(diff, mask, 15, 255, cv::THRESH_BINARY);
    double changedRatio = cv::countNonZero(mask) / (double)(mask.rows * mask.cols);

    return changedRatio < 0.01; // menos del 1% de píxeles cambiaron
}
